...
public class MailService {
    ...
    public void sendEmail(String to, String subject, String content, boolean isMultiPart, boolean isHTML) {
        ...
        try {
            ...
            message.setTo(to);
            message.setFrom("noreply@arilab.com");
            message.setSubject(subject);
            message.setText(content, isHTML);
            javaMailSender.send(mimeMessage);
        } catch (Exception mailException) {
            logger.warn("Email could not be sent to user '{}': {}", to, mailException.getMessage());
        }
    }
    ...
    public void sendActivationEmail(User user) {
        ...
        sendEmailFromTemplate(user,"email/activation","Expman User Activation");
    }

    @Async
    public void sendWelcomeEmail(User user) {
        logger.debug("Sending welcome email to '{}'", user.getEmail());
        sendEmailFromTemplate(user,"email/welcome","Welcome to Expman");
    }
}
